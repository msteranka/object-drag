...
public:
    ...
    VOID UpdateLastAccess(ADDRINT ptr, unsigned long t, THREADID tid, const CONTEXT *ctxt, size_t fragSize) {
        ...
        // TODO: Consider adding an option to allow users to disable retrieving 
        // the invocation point of the last access. Calling PIN_GetSourceLocation
    ...
    VOID KillLiveObjects() {
        ObjectData *d;
        Backtrace dummyBt;
        for (auto it = _liveObjects.begin(); !_liveObjects.empty(); it = _liveObjects.begin()) {
            d = it->second;
            DeleteObject(d->_addr, dummyBt, -1, -1);
        }
    }
